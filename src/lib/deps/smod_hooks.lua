	...
	self.set_badges = function(_, card, badges)
		if self.mstg_vars ~= nil then
			if self.mstg_vars.credits ~= nil then
				badges[#badges + 1] = (self.mstg_vars.credits.idea ~= nil) and
					 create_badge(
						 localize({ type = "variable", key = "k_mstg_credit_idea", vars = { self.mstg_vars.credits.idea } }),
						 HEX("867100FF"), G.C.WHITE, 0.8) or nil
				badges[#badges + 1] = (self.mstg_vars.credits.art ~= nil) and
					 create_badge(
						 localize({ type = "variable", key = "k_mstg_credit_art", vars = { self.mstg_vars.credits.art } }),
						 HEX("860000FF"), G.C.WHITE, 0.8) or nil
				badges[#badges + 1] = (self.mstg_vars.credits.code ~= nil) and
					 create_badge(
						 localize({ type = "variable", key = "k_mstg_credit_code", vars = { self.mstg_vars.credits.code } }),
						 HEX("00862eFF"), G.C.WHITE, 0.8) or nil
			end
		end
	end
